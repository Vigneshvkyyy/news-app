...
class EnvironmentCategory extends StatefulWidget {
  @override
  _EnvironmentCategoryState createState() => _EnvironmentCategoryState();
}

class _EnvironmentCategoryState extends State<EnvironmentCategory> {
  ...
  Future<void> fetchNews() async {
    final response = await http.get(
      Uri.parse(
          'https://gnews.io/api/v4/top-headlines?topic=environment&token=7256acfa88ecf5f4bc4688a64449aedf'),
    );
...
class EnvironmentDetailScreen extends StatefulWidget {
  final Map<String, dynamic> environmentNews;

   EnvironmentDetailScreen({required this.environmentNews});
      ...
      body: Container(
        ...
        child: Column(children: [
          ClipRRect(
            ...
            child: Image.network(
              widget.environmentNews["image"] ?? "No Images", 
              height: 180,
          ...
          Text(
            widget.environmentNews['title'] ?? 'No title',
          ...
          Row(
            children: [
              ...
              Text(
                widget.environmentNews['source']['name'] ?? 'No author',
                maxLines: 1,
                style: TextStyle(
                    fontSize: 14,
                    fontWeight: FontWeight.bold,
              ...
              Flexible(
                child: Text(
                  widget.environmentNews['publishedAt'] ?? 'No publishedAt',
                  maxLines: 1,
                  style: TextStyle(
                      fontSize: 14,
                      color: Colors.grey),
          ...
          Text(
            widget.environmentNews['description'] ?? 'No description',
            maxLines: 4,
            style: TextStyle(
                fontSize: 14,
                color: Colors.black),
          ...
          Text(
            widget.environmentNews['content'] ?? 'No content',
            maxLines: 4,
            style: TextStyle(
                fontSize: 14,
                color: Colors.black),
